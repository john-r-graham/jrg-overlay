...
inherit python-single-r1 flag-o-matic waf-utils multilib-minimal linux-info systemd pam tmpfiles git-r3
...
case "${PV}" in
	...
	9999)
		MY_PV="${PV}"
		MY_P="${P}"
		EGIT_REPO_URI="https://gitlab.com/samba-team/samba.git"
	...
	9998)
		MY_PV="${PV}"
		MY_P="${P}"
		EGIT_REPO_URI="file:///home/jgraham/Projects/Gentoo/samba-nss-winbind-experiments"
		REFS="refs/heads/nss_winbind_experiments"
		KEYWORDS="~alpha ~amd64 ~arm ~arm64 ~hppa ~loong ~mips ~ppc ~ppc64 ~riscv ~sparc ~x86"
		;;
	# Normal upstream tarball releases.
	*)
		MY_PV=$(ver_cut 1-3 ${PV/_rc/rc})
		MY_P="${PN}-${MY_PV}"
		if [[ ${PV} == *_rc* ]]; then
...
multilib_src_install() {
	...
	if multilib_is_native_abi ; then
		...
		# Add winbind to the list of daemons to start if winbind has been built
		if use winbind ; then
			sed -e '/^daemon_list=/ s/smbd/& winbind/' -i "${ED}/etc/conf.d/samba" || die
		fi
