...
DESCRIPTION="OpenRC manages the services, startup and shutdown of a host; with JRG net-online patch"
HOMEPAGE="https://github.com/openrc/openrc/"

if [[ $(ver_cut 4) == p ]] ; then
	MY_PV=$(ver_cut 1-3)
	PATCH_LEVEL=$(ver_cut 5)
else
	MY_PV=${PV}
	PATCH_LEVEL="0"
fi
...
if [[ ${MY_PV} =~ ^9{4,}$ ]]; then
	...
	SRC_URI="https://github.com/OpenRC/openrc/archive/${MY_PV}.tar.gz -> ${MY_P}.tar.gz"
...
if [[ "$PATCH_LEVEL" == "1" ]] ; then
	   PATCHES+=( "${FILESDIR}/openrc-0.62.10-net-online-tun-tap-support.patch" )
fi
...
pkg_postinst() {
	...
	if [[ "$PATCH_LEVEL" == "1" ]] ; then
		ewarn "JRG tun/tap identification patch applined to the net-online init script."
		ewarn "See the pull request for details: https://github.com/OpenRC/openrc/pull/908"
	fi
}
