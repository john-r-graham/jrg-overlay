...
pkg_postinst() {
	...
	if use down-root ; then
		ewarn ""
		ewarn "WARNING: The down-root USE flag (and the plugin it builds) are"
		ewarn "obsolete. openvpn now has built-in support to regain root privileges"
		ewarn "during shutdown / cleanup. You should remove the down-root USE flag"
		ewarn "and any drop-root directive you have in your openvpn.conf files."
	fi
}
